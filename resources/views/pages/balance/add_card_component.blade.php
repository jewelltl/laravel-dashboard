                        ...
                        @foreach(Auth::user()->PaymentMethods as $index=>$method)
                            <div class="card has-border {{$method->main == 1 ? 'main-payment' : ''}} sortable" data-id="{{$method->customer_id}}">
                              <div class="card-body ">
                                ...
                                @if(isset( $customer[0]->maskedNumber ))
                                    <h4 class="card-title">
                                        {{Auth::user()->name}}
                                        <small><img src="{{ $customer[0]->imageUrl }}"> &nbsp;{{ $customer[0]->maskedNumber }}</small>
                                    </h4>
                                    <p>
                                        Expires: {{ $customer[0]->expirationDate}} &nbsp; 
                                    </p>
                                @else
                                    <h4>{{Auth::user()->name}}</h4>
                                    <p>
                                        <img src="{{ $customer[0]->imageUrl }}"> &nbsp;{{ $customer[0]->email }}
                                    </p>
                                @endif
                              </div>
                            </div>
